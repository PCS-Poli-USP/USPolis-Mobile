...
export const ClassSelection = ({ onClose }: { onClose: () => void }) => {
  ...
  useEffect(() => {
    if (courseProgram) {
      ...
      const availableDisciplinesWithClasses = newAvailableDisciplines.filter(
        (c) => c.classes.length,
      )
  ...
  const handleToggleClassOnSchedule = () => {
    const selectedDisciplines = availableDisciplines.filter(
      (discipline) => discipline.selected,
    )
    const selectedClasses = selectedDisciplines.map(
      (discipline) => discipline.selectedClass,
    )

    logger.logEvent('Aulas adicionadas automaticamente', {
    ...
    <>
      <Box backgroundColor="graySeven" borderBottomColor="transparent">
        <VStack alignItems="center" marginBottom={'l'}>
          ...
          >
            {isLoadingCourses && <ActivityIndicator />}
                ...
                renderItem={({ item }) => (
                  <MaybeTouchableSelectableDiscipline
                    ...
                    handleSelectClass={(classId) =>
                      handleSelectClass(item.id, classId)
                    }
                  />
                )}
              />
        ...
        <VStack backgroundColor="graySeven" marginBottom="m">
          {!isLoadingCourses && !!availableDisciplines.length && (
            <Button
              ...
              disabled={
                !availableDisciplines.some((discipline) => discipline.selected)
              }
            />
          )}
          <Box mb="s" />
...
const MaybeTouchableSelectableDiscipline = (
  props: ISelectableDisciplineProps,
) => {
  if (!props.selected) {
    return (
  ...
  return <SelectableDiscipline {...props} />
}

const SelectableDiscipline = ({
  name,
  selected,
  classes,
  handleSelect,
  ...
  return (
    ...
    >
      ...
      <Box
        ...
        marginRight="s"
      >
        <Checkbox selected={selected} onPress={() => handleSelect()} />
      </Box>
    </Box>
  )
}
