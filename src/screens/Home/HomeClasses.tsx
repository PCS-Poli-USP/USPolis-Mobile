...
import { HStack, VStack, Text, Box, FlatList, Heading, useTheme } from "native-base"
...
export const HomeClasses = ({ buildingFilter, nameFilter }: HomeClassesProps) => {
  ...
  const filteredClasses = useMemo(() => {
    ...
    if (nameFilter) {
      classesFiltered = classesFiltered?.filter((c) => c.subject_name.includes(nameFilter))
    }

    if (buildingFilter) {
      classesFiltered = classesFiltered?.filter(c => {
        const classesInBuilding = c.schedule.filter(s => s.building === buildingFilter)
  ...
  return (
    <VStack>
      <HStack flex={1} justifyContent={'space-between'} mb={4}>
        <Text color='gray.200' fontWeight={'bold'}>Aulas</Text>
      ...
      {!isLoadingClasses && filteredClasses.map((sclass) => (
        <HomeClassCard sclass={sclass} key={sclass.id} />
      ))}
    </VStack>
  )
}
...
export const HomeClassCard = ({ sclass }: HomeClassCardProps) => {
  ...
  const handleClassPress = () => {
    console.log('openning class details')
  }
    ...
    <TouchableOpacity onPress={handleClassPress}>
      ...
      >
        <VStack flex={1} mr={3}>
          ...
          <Text color='gray.200' mb={1} numberOfLines={2}>Turma {sclass.class_code.slice(-2)} - {buildings}</Text>
          <Text color='gray.100' numberOfLines={2} fontFamily={'heading'}>{classRooms}</Text>
        </VStack>
